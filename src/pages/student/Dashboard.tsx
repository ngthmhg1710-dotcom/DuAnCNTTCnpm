        ...
        <div className="col-span-2 space-y-5">
          ...
          <div className="card">
            ...
            <div className="divide-y divide-slate-50">
              {upcoming.map(act => (
                <div key={act.id} className="px-5 py-4 flex items-start justify-between hover:bg-slate-50/50">
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm text-slate-800 truncate">{act.name}</div>
                  ...
                  <button onClick={() => navigate(`/student/activities/${act.id}`)} className="btn-secondary text-xs ml-4 shrink-0">Xem chi tiết</button>
                </div>
              ))}
            </div>
          </div>
